...
export const checkAuthenticated = () => async (dispatch) => {
	...
	if (localStorage.getItem('access')) {
		...
		try {
			const res = await axios.post(REQUEST_URL + `/auth/jwt/verify/`, body, config);
...
export const login = (email, password) => async (dispatch) => {
	...
	console.log(body);
	try {
		const res = await axios.post(REQUEST_URL + `/auth/jwt/create/`, body, config);
		dispatch({
...
export const update_profile = (phone, address, state, postcode) => async (dispatch) => {
	dispatch({
		type: types.REQUEST_START,
	});
	const config = {
